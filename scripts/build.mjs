...
const staleMappings = [...licenseCatalog.keys()].filter((name) => !bundledPackages.includes(name));
if (staleMappings.length > 0) {
  throw new Error(`stale full license text mappings: ${staleMappings.join(", ")}`);
}
const noticeSections = await Promise.all(bundledPackages.map(async (name) => {
  const entry = licenseCatalog.get(name);
  if (entry === undefined) {
    throw new Error(`missing full license text mapping for bundled package ${name}`);
  }
  const [license, path] = entry;
