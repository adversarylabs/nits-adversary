...
export function createApp() {
    ...
    app.rule("nits.redundant_secret_masking", async (ctx) => {
        ...
        for (const source of sources) {
            ...
            for (let i = 0; i < lines.length; i++) {
                const line = lines[i] ?? "";
                const guarantee = guarantees.find((candidate) => candidate.path === source.path &&
    ...
    for (const match of content.matchAll(signature)) {
        ...
        for (const sanitizerCall of body.matchAll(call)) {
            ...
            if (sanitizer === undefined ||
                ...
                !parameterNames.some((parameter) => new RegExp(`\\b${escapeRegex(parameter)}\\b`).test(args)) ||
...
function isTopLevelNestedCall(body, sanitizerOffset, line, sanitizer) {
    ...
    const depth = [...prefix].reduce((value, character) => value + (character === "{" ? 1 : character === "}" ? -1 : 0), 0);
    if (depth !== 0)
        return false;
