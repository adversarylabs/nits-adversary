#!/usr/bin/env node
import { createRequire as __nitsCreateRequire } from 'node:module'; const require = __nitsCreateRequire(import.meta.url);
...
async function walkRelative(repoPath, ignore) {
  ...
  async function visit(relativeDir) {
    ...
    for (const entry of entries) {
      const rel2 = relativeDir === "" ? entry.name : `${relativeDir}/${entry.name}`.replaceAll("\\", "/");
      if (entry.isDirectory()) {
...
var TODO_LANDMINE = /\b(TODO|FIXME|XXX|HACK)\b(?![^\n]{0,80}\b(https?:\/\/|#[0-9]+|[A-Z]+-\d+)\b)/i;
var UNFINISHED_RENAME_PAIRS = [
...
function createApp() {
  ...
  app2.rule("nits.todo_landmine", async (ctx) => {
  ...
  app2.rule("nits.unfinished_rename", async (ctx) => {
    const sources = await loadScopedSources(ctx);
    for (const source of sources) {
      if (shouldSkipPath(source.path)) continue;
      const content = source.content || "";
  ...
  app2.rule("nits.stale_comment_marker", async (ctx) => {
    const sources = await loadScopedSources(ctx);
  ...
  app2.rule("nits.redundant_secret_masking", async (ctx) => {
    const sources = await loadScopedSources(ctx);
    const guarantees = sources.flatMap(
      (source) => findMaskingGuarantees(source.path, source.content || "")
    );
    let emitted = 0;
    for (const source of sources) {
      if (emitted >= 3) break;
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const guarantee = guarantees.find(
          (candidate) => candidate.path === source.path && !(offset >= candidate.start && offset < candidate.end) && containsNestedMaskingCall(line, candidate)
        );
        if (guarantee === void 0) {
          offset += line.length + 1;
          continue;
        }
        if (!isEligibleLine(source, i + 1) && !isEligibleLine(source, guarantee.line)) {
          offset += line.length + 1;
          continue;
        }
        ctx.finding({
  ...
  app2.rule("nits.dockerfile_run_indentation", async (ctx) => {
    const sources = await loadScopedSources(ctx, {
  ...
  app2.rule("nits.diagnostic_operation_mismatch", async (ctx) => {
    const sources = await loadScopedSources(ctx, {
    ...
    for (const source of sources) {
      ...
      if (operation === void 0 || shouldSkipErrorDomainPath(source.path)) continue;
...
var PATH_OPERATIONS = ["create", "enable", "disable", "delete", "remove", "update", "login", "logout"];
...
function isEligibleLine(source, line) {
  return source.status === "repository" || source.status === "added" || source.changedLines.has(line);
}
...
function shouldSkipPath(path) {
  ...
  return p.includes("/.git/") || p.includes("node_modules/") || p.includes("/dist/") || p.endsWith(".gitkeep") || p.endsWith("package-lock.json");
}
...
function findDockerfileRunIndentationOutliers(content) {
  ...
  for (let runLine = 0; runLine < lines.length; runLine++) {
    ...
    for (let current = runLine + 2; current < end; current++) {
      ...
      if (previous.trim() === "" || line.trim() === "" || next.trim() === "" || /^\s*#/.test(line) || /^[ \t]*\t/.test(previous) || /^[ \t]*\t/.test(line) || /^[ \t]*\t/.test(next)) {
        continue;
      }
      const expected = leadingSpaces(previous);
      const actual = leadingSpaces(line);
      if (expected === 0 || leadingSpaces(next) !== expected || Math.abs(actual - expected) !== 1) {
        continue;
      }
...
function isStructurallyIndentedRunBlock(lines) {
  ...
  return lines.some((line) => {
    ...
    return /^(?:if|then|elif|else|fi|for|while|until|case|esac|select|do|done)\b/.test(shell) || /^[({]/.test(shell);
  });
}
...
function findMaskingGuarantees(path, content) {
  const guarantees = [];
  const signature = /(?:^|\n)[^\n{};]*?\b([A-Za-z_]\w*)\s*\(([^()\n]*)\)\s*(?:[^\n{]*)\{/g;
  const controlWords = /* @__PURE__ */ new Set(["catch", "for", "if", "switch", "while"]);
  for (const match of content.matchAll(signature)) {
    const recorder = match[1];
    const parameters = match[2];
    if (recorder === void 0 || parameters === void 0 || controlWords.has(recorder) || !RECORDER_NAME.test(recorder)) {
      continue;
    }
    const open2 = (match.index ?? 0) + match[0].lastIndexOf("{");
    const end = matchingBrace(content, open2);
    if (end === void 0) continue;
    const body = content.slice(open2 + 1, end);
    ...
    for (const sanitizerCall of body.matchAll(call)) {
      ...
      if (sanitizer === void 0 || args === void 0 || !SECRET_MASKER_NAME.test(sanitizer) || !parameterNames.some(
        (parameter) => new RegExp(`\\b${escapeRegex(parameter)}\\b`).test(args)
      ) || !isTopLevelNestedCall(body, sanitizerOffset, line, sanitizer)) {
        continue;
      }
      const sourceLine = content.slice(0, open2 + 1).split("\n").length + sanitizerLine - 1;
      guarantees.push({ recorder, sanitizer, path, line: sourceLine, start: open2, end });
    }
  }
  return guarantees;
}
function isTopLevelNestedCall(body, sanitizerOffset, line, sanitizer) {
...
function extractParameterNames(parameters, path) {
  return parameters.split(",").map((parameter) => parameter.trim().split("=")[0]?.trim() ?? "").map((parameter) => {
  ...
  }).filter((parameter) => parameter !== void 0);
}
function matchingBrace(content, open2) {
  let depth = 0;
  for (let i = open2; i < content.length; i++) {
    if (content[i] === "{") depth++;
    if (content[i] === "}") depth--;
...
function containsNestedMaskingCall(line, guarantee) {
